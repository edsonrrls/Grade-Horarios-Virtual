...
$pdf->SetFont('Arial','I',11); 
        $pdf->Cell(90,8,utf8_decode('DISCIPLINAS:'),1,0,'C');
        $pdf->Cell(90,8,utf8_decode('CURSOS:'),1,1,'C');
    while ($dado = $condisc->fetch_array())
    {
$pdf->SetFont('Arial','',10);
        $pdf->Cell(100,6,utf8_decode($dado["nome_disc"]),1,0,'C'); 
        $pdf->Cell(90,6,utf8_decode($dado["curso_disc"]),1,1,'C');
    }

$pdf->SetFont('Arial','B',11); 
        $pdf->Cell(95,8,utf8_decode('Segunda-Feira'),1,0,'C');
        $pdf->Cell(95,8,utf8_decode('Terça-Feira'),1,1,'C');
...
$pdf->SetFont('Arial','',8); 
    ...
    {
        $pdf->Cell(10,8,utf8_decode($dado["hora_inicio"]),1,0,'C');
        $pdf->Cell(10,8,utf8_decode($dado["hora_termino"]),1,0,'C');
        $pdf->Cell(75,8,utf8_decode($dado["nome_disc"]),1,1,'C');
    }

     while ($dado = $con3->fetch_array())
    {
        $pdf->Cell(10,5,utf8_decode($dado["hora_inicio"]),1,0,'C');
        $pdf->Cell(10,5,utf8_decode($dado["hora_termino"]),1,0,'C');
        $pdf->Cell(75,5,utf8_decode($dado["nome_disc"]),1,1,'C');
    }


$pdf->SetFont('Arial','B',11); 
        $pdf->Cell(95,8,utf8_decode('Quarta-Feira'),1,0,'C');
        $pdf->Cell(95,8,utf8_decode('Quinta-Feira'),1,1,'C');
...
$pdf->SetFont('Arial','',8);  
    ...
    while ($dado = $con6->fetch_array())
    {
        $pdf->Cell(10,8,utf8_decode($dado["hora_inicio"]),1,0,'C');
        $pdf->Cell(10,8,utf8_decode($dado["hora_termino"]),1,0,'C');
        $pdf->Cell(44,8,utf8_decode($dado["nome_disc"]),1,1,'C');
    }
